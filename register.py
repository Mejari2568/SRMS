...
#import pymysql # pip install pymysq1
import sqlite3
import os
class Register:
    def __init__(self,root):
        ...
        btn_login=Button(self.root,text="Sign In",font=("times new roman",15),bd=0,cursor="hand2",command=self.login_window).place(x=200,y=460,width=180)
        ...
        if self.txt_fname.get()=="" or self.txt_contact.get()=="" or self.txt_email.get()=="" or self.cmb_quest.get()=="Select" or self.txt_answer.get()=="" or self.txt_password.get()=="" or self.txt_cpassword.get()=="":
            ...
            try:
                ...
                if row!=None:
                    ...
                    cur.execute("insert into employee (fname,lname,contact,email,question,answer,password,cpassword)values(?,?,?,?,?,?,?,?)",
                                (self.txt_fname.get(),
                                 self.txt_lname.get(),
                    ...
                    con.commit()
                    con.close()
                    messagebox.showinfo("Success","Registration Successsful",parent=self.root)
                    self.clear()
                    self.login_window()
            except Exception as es:
                print(es)
                messagebox.showinfo("success",str(es),parent=self.root)
